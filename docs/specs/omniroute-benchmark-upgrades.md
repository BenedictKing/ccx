...
> 状态：**部分实现**（2026-09-01 定稿；§4 Guardrails 最小集 + §5 路由预演已落地，各拆独立 spec；其余项规划中）。各项落地后应拆出独立 spec 并回填状态。
...
## 5. 方向 D：路由预演升级（请求体直喂 route preview）【已实现】
...
| 3 | A. 配额分级调度 | 调度体验最大增量，与拥挤度合流 | 单测（桶懒重置/真相分级/降级排序）+ 饱和场景集成测试 | 规划中 |

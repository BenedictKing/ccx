...
func TestHandleSuccess_PreservesPreviousResponseID(t *testing.T) {
	...
	if _, err := handleSuccess(c, resp, provider, nil, "", "responses", envCfg, sessionManager, time.Now(), originalReq, []byte(`{"model":"gpt-5","input":"hello"}`), common.StreamPreflightTimeouts{}); err != nil {

...
func TestDeriveQualityTier(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "claude fable（直测 69.9）→ high",
			family:  ModelFamilyClaude,
			modelID: "claude-fable-5",
			want:    QualityTierHigh,
		},
		{
			name:    "gpt-5.6（直测 72.7）→ high",
			family:  ModelFamilyOpenAI,
			modelID: "gpt-5.6-sol",
			want:    QualityTierHigh,
		},
		{
			name:    "gpt-5.5（直测 67.9）→ high",
			family:  ModelFamilyOpenAI,
			modelID: "gpt-5.5",
			want:    QualityTierHigh,
		},
		{
			name:    "gpt-5.4（直测 51.8）→ normal",
			family:  ModelFamilyOpenAI,
			modelID: "gpt-5.4",
			want:    QualityTierNormal,
		},

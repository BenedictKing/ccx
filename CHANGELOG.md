...
- **渠道兼容性开关统一自学习** - 渠道级兼容性配置从「用户手工勾选的静态布尔」改为「运行时按渠道-Key-模型自动学习并持久化的能力事实」，不再需要用户先踩坑再去翻设置。新增 `ChannelCompatCache`（落盘 `.config/channel_compat.json`，TTL 24h，键 `channelUID:keyHash:model`）统一记忆六项兼容能力；`NormalizeNonstandardChatRoles`/`StripImageGenerationTool`/`CodexNativeToolPassthrough`/`StripEmptyTextBlocks`/`PassbackReasoningContent`/`PassbackThinkingBlocks` 全部迁移为 `*bool` 并按 `用户当前显式设置 > 学习结论 > 手工配置种子 > 静态默认` 合成，`RuntimeUpstreamForAutoManagedProvider` 相应改为清 nil 而非 false 以免自动托管渠道永远学不会。按信号强度分三条学习路径：有硬 400 信号的走 `CompatTraitFromError` 错误驱动（识别后同 Key 立即重试，改写在下一轮注入），其中 Codex `/v1/responses` 打到不支持 `developer` role 的上游时新增 `DowngradeDeveloperRoleToSystem` 窄改写（降 `system` 而非 `user`，保留开发者指令语义）；仅能靠响应形态判断的 `passbackReasoningContent`/`stripEmptyTextBlocks` 复用渠道诊断探针，改为首次遇到该组合时 singleflight 后台自动探测一次，替代原先要用户手工点按钮；图片生成受限从「直接拉黑 (Key,模型)」改为「先学习剥离图片工具并重试，剥离后仍失败才回落拉黑」。所有识别规则采用双条件门控（正则命中 + 请求侧确实携带对应结构），且 429/5xx/超时一律不参与兼容性学习，避免把容量问题学成能力缺失
- **历史手工兼容配置升级合并进自学习** - 老用户手工勾选过的兼容开关不再永久压过自动学习，否则上游能力变化后仍需人工再改一次。启动时 `migrateManualCompatSwitchesToSeeds` 一次性把手工值搬到渠道级 `compatSeeds` 并将原字段置 nil，作为学习结论缺失时的兜底证据：迁移不改变可感知行为，但一旦学到上游真实结论即自动让位，此后用户无需手工干预。种子无 TTL（用户意图不过期）且与 TTL 24h 的学习记忆分开存储，避免种子 24 小时后凭空消失；保留 `false` 值（显式关闭同样是观察证据）；迁移幂等，用户之后重新手工设置的值仍保持最高优先级；自动托管渠道连种子一并清空，兼容性完全由厂商原生默认加运行时学习决定

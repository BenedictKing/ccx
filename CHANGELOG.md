...
- **新增模型列表手工返回模式** - 现在可在 Web UI 按渠道单独配置 `/v1/models` 的返回方式，在“上游拉取”之外新增“手工返回”模式，可直接维护一组模型列表返回给客户端与管理页模型查询，不再依赖上游是否支持 `/v1/models`

### Changed

- **模型列表返回策略与 Claude 透传配置解耦** - 新增独立的 `modelsResponseMode` 与 `manualModels` 渠道配置，确保手工介入模型列表时不会影响 Claude 的流式透传、sub2api 认证透传和严格请求透传等既有行为
